...
def classify(msg: str) -> str:
    ...
    if any(w in m for w in ["continue", "reprendre", "reprends", "continuer"]):
        return "CONTINUE"
    if m.startswith("modifier") or m.startswith("modify") or m.startswith("update") or m.startswith("nouveau"):
        return "MODIFY_FOLDER"
    if any(w in m for w in ["ajouter", "add", "creer", "créer", "nouveau", "modifier", "modify", "update", "mettre a jour", "mettre à jour"]):
        return "CREATE"
...
def run_goal(index: dict, goal_id: int, template_obj: dict, req_paths: list):
    ...
    while not state.get("done"):
        ...
        # 1) Tentative extraction multi-champs (peut remplir plusieurs champs à la fois)
        apply_user_message_to_facts(state["facts"], template_obj, user_msg)
        auto_fill(state["facts"])
        

        # 2) Si le champ demandé est toujours manquant, on applique la réponse au champ demandé (fiable)
        miss_after = missing_paths(state.get("facts", {}), template_obj, req_paths)
        #pour sauter les infos donnees par le client
        if next_path not in miss_after:
            save_json(state_path,state)
            continue
        ok=apply_single_field_answer(state["facts"],template_obj,next_path,user_msg)
        if ok:
            auto_fill(state["facts"])

        
        if not ok:
                print("AGENT> Je n’arrive pas à enregistrer cette valeur. Réessaie.\n")
    ...
    while True:
        ...
        if intent == "CONTINUE":
            ...
            if target_folder:
                ...
                if not match:
                    print("Agent> Dossier introuvables. \n")
                    continue
                gid=match["goal_id"]
            else:
