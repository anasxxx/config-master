-- ============================================================
-- DIAGNOSTIC v2: Drill into LOAD_BANK_PARAMETERS (step 6)
-- to find the EXACT Oracle error.
--
...
BEGIN
    -- Get currency & country (same as before, known to work)
    ...
    DBMS_OUTPUT.PUT_LINE('currency_alpha=[' || v_cur_alpha || '] country_alpha=[' || v_cty_alpha || ']');

    -- Run prerequisite steps (known to pass)
    ...
    -- ===== Now replicate LOAD_BANK_PARAMETERS step by step =====
    ...
    -- 6c. CENTER insert
    BEGIN
        SELECT MAX(TO_NUMBER(NVL(CENTER_CODE, 0))) + 1 INTO v_sequence_id FROM CENTER WHERE CENTER_CODE < '21';
    ...
    EXCEPTION WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('6f. PCARD_TASKS_EXEC_GROUP_BANK FAILED: ' || SQLERRM);
        ROLLBACK; RETURN;
    END;

    DBMS_OUTPUT.PUT_LINE('=== ALL 6 SUB-STEPS PASSED ===');
